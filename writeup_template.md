...
Following Udacity's suggestion I implemented NVIDIA's self-driving model architecture as shown below:

![alt text][image1]

In my testing I found that there would be generally low mean squared error on the training set but a high mean squared error on the validation set, this implied that the model was overfitting. One of the ways I modified the model was to add dropout of 50% to generally the model.

The final step was to run the simulator to see how well the car was driving around track one. There were a few spots where the vehicle fell off the track near the dirt road, to improve the driving behavior in these cases, I ....
